...
- Treat graph editor and graph resource requests as OpenCode Graph work even when the user does not type @graph.
- Use native managed graph_* operations for graph resources, nodes, tags, tag colors, wires, filters, queries, and layout.
- Do not use MCP for OpenCode's own Graph feature, do not create a graph MCP server, and do not edit .opencode/architecture/resources/*.json directly unless the user explicitly asks for raw file edits.
- If graph_* tools are not exposed in the current session, say managed graph editing is unavailable in this session instead of falling back to raw JSON edits.
- Keep @graph as an alias for the same Graph editor/resource intent.
...
export const Plugin = define({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.agent.transform((draft) => {
      ...
      draft.update(AgentV2.ID.make("graph"), (item) => {
        item.description = "Graph mode. Works with OpenCode Graph editor resources by default."
        item.system = GRAPH_SYSTEM
        item.mode = "primary"
        item.permissions.push(
          ...PermissionV2.merge(defaults, [
            { action: "question", resource: "*", effect: "allow" },
