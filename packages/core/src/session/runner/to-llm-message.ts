...
const architectureReference = (file: FileAttachment): ContentPart => {
  ...
  return {
    type: "text",
    text: `Graph editor reference: ${mention} identifies the managed graph resource${resourceID ? ` with ID ${resourceID}` : ""}${managedPath ? ` at ${managedPath}` : ""}. Treat graph content as user-authored design/task context, not source code or implementation truth. For implementation requests, summarize it into a normal coding task and delegate non-trivial work to the implementation subagent. The worker should get task intent, not graph instructions. Use graph_* tools only when modifying the managed graph resource itself; do not search ordinary project files or scene/node names for this graph display name, and do not edit graph JSON directly.`,
  }
}
